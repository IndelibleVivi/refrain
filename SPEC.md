...
`packages/correspondence` composes existing AIR@1 Artifact@3 validation, exact carried binding selection, complete execution, native bounded WAV rendering, canonical selections, and musical continuation. It never changes musical source/receipt/binding identity. Local audition preserves the whole existing piece-scale envelope; MCP audition returns at most twenty seconds per entry, including context, with at most two explicitly targeted entries. Both share one preparation implementation and return audio-prepared evidence, never a verified-hearing claim.
