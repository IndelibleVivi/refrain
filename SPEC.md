...
- MCP surface: stateless `hum` and `audition` actions plus one read-only App resource.
...
Browser and MCP views retain this entire document as their save and handoff authority. Selecting a carried binding changes only local playback state, stops/reset playback without auto-play, and does not change the saved default or remove other bindings or evidence. Valid unbound/ambiguous documents remain inspectable. Generated current presentation URLs use the existing digest-bound portable artifact fragment; historical Presentation@0/@1/@2 readers remain format-exact.
...
`audition` prepares exact audio material under the separate correspondence contract; it is not another composer or playback controller. There is no separate compose, play, motif-lookup, or presentation-fetch tool. Playback state is ephemeral view state. Canonical continuity is passed in the next tool call rather than stored inside the view or server.
...
Interactive playback implements progressive preparation: verify/decode the opening closure only after a user gesture, schedule with bounded lookahead, prefetch later first-use assets, and support pause/resume, stop, restart, seek, current/total time, and section jump. Seek reconstructs sustained, loop, release, sample-offset, oscillator-phase, and upcoming state without audible replay from zero. Offline reference rendering shares a deterministic block kernel across browser workers and Node; bounded two-pass Node WAV never allocates a whole-piece PCM array. Byte-exact reproducibility applies to this pinned offline renderer, while browser realtime paths owe semantic parity unless they emit bytes under that same contract.
...
The approved 2026-09-12 extension supports expression addressed to people or other agents, self-listening, optional free responses, local selected sharing, reception, and musical replies. It introduces no quality score, hidden listening model, automatic improvement loop, community account, or delivery service. [CORRESPONDENCE.md](docs/CORRESPONDENCE.md) defines the exact audition/response/share contracts, commands, evidence boundaries, and MCP transport limits.

`packages/correspondence` composes existing AIR@1 Artifact@3 validation, exact carried binding selection, complete execution, native bounded WAV rendering, canonical selections, and musical continuation. It never changes musical source/receipt/binding identity. A complete single-entry local audition may be sliced repeatedly from its frozen native WAV without re-resolving sound, loading assets, or rerendering; canonical source compilation may be used only to map an exact musical target back to frames. A clipped or paired packet cannot pretend it retains missing whole-render history. Local audition preserves the whole existing piece-scale envelope; MCP audition returns at most twenty seconds per entry, including context, with at most two explicitly targeted entries. Both share one preparation implementation and return audio-prepared evidence, never a verified-hearing claim.

Subjective response identity and musical lineage remain distinct. A response may be retained without critique or revision. Sharing explicitly selects frozen audio, optional whole source artifact and selected responses; receipt of a package does not grant its text host-instruction authority. Exact source-free audio remains playable without original assets, while formal `hum reply` needs the full parent artifact.
