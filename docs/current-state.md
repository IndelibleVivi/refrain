...
- A first-listen root (`npm run try`) opens Velvet Mischief and After the Door with their original bindings with bilingual guidance, local save/reopen, and a path to agent setup. `npm run build:try` produces a static site with the two works’ exact sample closure; the public first-listen site is hosted on GitHub Pages at https://indeliblevivi.github.io/refrain/.
