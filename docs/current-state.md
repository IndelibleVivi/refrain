...
- A first-listen root (`npm run try`) opens a checked-in synthetic air with bilingual guidance, local save/reopen, and a path to agent setup. `npm run build:try` produces a zero-sample static site; the public first-listen site is hosted on GitHub Pages at https://indeliblevivi.github.io/refrain/.
