...
Run `npx vitest run apps/presentation/src/correspondence.test.ts packages/mcp-server/src/audition.test.ts` for exact full-render/clip equality, incoming sustain, tempo-aware comparison, stale selection rejection, native media/measurement integrity, optional response binding, selected source-free sharing, regular-file closure, fresh-process reception and formal reply, and strict MCP schema/audio-content behavior. Run the shared CLI/audio/MCP/selection/HTTP smokes, export reproduction, complete-piece rendering and browser playback lanes after changes to common rendering or delivery.
