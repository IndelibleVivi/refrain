...
The shared Player / first-listen browser lane (`npm run test:try`) builds the static presentation and serves it below `/refrain/`. Desktop and 390px checks cover pre-gesture silence, the two featured works’ exact sampled playback, four base appearance changes, language synchronization, retained selection/focus/position, exact artifact download and local reopen, invalid-file recovery, and broken-link errors. It additionally exercises personal playlist import, add/remove/reorder and complete-archive save/reopen with listening choices, work-presentation precedence and explicit local preference recovery, all four composers’ image material layers, dialog keyboard/focus behavior, persisted playlist mode, one real natural-end continuation, an imported work whose exact asset closure is already published, per-theme scalar/local-image persistence and reset during playback, zero-upload demo-link copy and fresh receive, omission of local image data, and artifact-hash tamper rejection. It rejects pre-gesture sample requests and requests outside the static origin. These are local Chromium observations, not public-host deployment, physical-device share-sheet behavior, visual/listening acceptance, or static-package publication. The MCP language cases also traverse every base appearance while retaining exact host export bytes.
