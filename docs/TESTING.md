...
The first-listen browser lane (`npm run test:try`) builds the static presentation and serves it below `/refrain/`. Desktop and 390px checks cover pre-gesture silence, the two featured works’ exact sampled playback, four appearance changes, language synchronization, retained selection/focus/position, exact artifact download and local reopen, invalid-file recovery, and broken-link errors. It rejects pre-gesture sample requests and requests outside the static origin, and exercises an unavailable non-featured sampled artifact without rebinding. These are local browser observations, not public-host deployment or listening acceptance. The MCP language cases also traverse every appearance while retaining exact host export bytes.

## Correspondence evidence

Run `npx vitest run apps/presentation/src/correspondence.test.ts packages/mcp-server/src/audition.test.ts` for exact full-render/clip equality, repeated no-rerender slicing from one frozen native performance, local time mapping, canonical output/input packet isolation across path aliases, incoming sustain, post-render cancellation cleanup, tempo-aware comparison, stale selection rejection, native media/measurement integrity, optional response binding, selected source-free sharing, bounded streaming share-directory closure, copied-byte verification, hardlink/symlink refusal, fresh-process reception and formal reply, and strict MCP schema/audio-content behavior. Run the shared CLI/audio/MCP/selection/HTTP smokes, export reproduction, complete-piece rendering and browser playback lanes after changes to common rendering or delivery.

These tests prove actual WAV preparation and protocol content, not named-host model audio input, musical perception, listening acceptance, deployment, or external delivery. A receiver can use frozen sound without original assets; exact re-rendering still requires its separate sound closure.
