...
The Player lane also starts its real Vite development entry on an ephemeral loopback port for one StrictMode regression: manual next/previous must start the newly mounted work after React's setup/cleanup/setup replay, and Stop must return it to ready. A static production preview alone cannot reject a command incorrectly consumed by the disposed first mount. Natural-end tests retain the exact rendered tail: the sampled demo proves continuation, while a zero-asset short-tail fixture proves the last sequential entry stops without wrapping.

The shared Player / first-listen browser lane (`npm run test:try`) builds the static presentation and serves it below `/refrain/`. It drives the listening-room layout: the `.refrain-wordmark` header, the hero-over-score-over-transport hierarchy with one shared `.page-transport`, and the Play control inside the first 900px viewport. The playlist is a native `dialog` (`#player-playlist`) opened by `[aria-controls=player-playlist]`, closed on arrival, closed by Escape with focus returned to its trigger, and closed by choosing a work on both desktop and 390px; saving or editing reopens it, and the file input lives inside it, so every import opens the sheet first. Technical details are one collapsed `details.product-details` (`Inside this air` / `走进这首 air` on the URL surface; the MCP surface keeps `About this air`) that now owns the former passage/motif panels, the motif map, and exact selection; the multiple-binding chooser is a collapsed `details.refrain-renderer__performance`; and the About Refrain guidance is its own native dialog. Desktop and 390px checks cover pre-gesture silence, a visible and substantial score rather than a decorative sliver, the playlist not shrinking or covering the music when closed, the two featured works’ exact sampled playback, four base appearance changes, language synchronization, retained selection/focus/position, exact artifact download and local reopen, invalid-file recovery, and broken-link errors. It additionally exercises personal playlist import, add/remove/reorder and complete-archive save/reopen with listening choices, transport previous/next and a real natural-end continuation, work-presentation precedence and explicit local Work/My appearance isolation, all four composers’ image material layers, dialog keyboard/focus behavior, persisted playlist mode, an imported work whose exact asset closure is already published, per-theme scalar/local-image persistence and reset during playback, zero-upload demo-link copy and fresh receive, omission of local image data, and artifact-hash tamper rejection. It rejects pre-gesture sample requests and requests outside the static origin. These are local Chromium observations, not public-host deployment, physical-device share-sheet behavior, visual/listening acceptance, or static-package publication. The MCP language cases also traverse every base appearance while retaining exact host export bytes.
