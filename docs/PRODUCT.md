...
The canonical Player opens personal works and portable playlists, with add/remove/reorder, previous/next, four playback modes and explicit save/reopen. First listen is that same Player preloaded with two works and brief guidance, not a second product or a website builder. The host agent authors music and may choose a work presentation through the existing CLI or playlist metadata; the listener hears, inspects, selects passages, manages a local queue and may explicitly switch to device-local appearance. Work presentation wins by default and never silently merges a listener’s old photo/colors. Appearance settings stay in a closed drawer, with presets before fine controls. Local photos enter the existing composers’ material layers without changing their geometry; crop/strength/blur are adjustments, not a promise of perfect arbitrary-photo harmony. No image-generation integration, additional MCP payload, upload service or official website is introduced. The same Canvas leads back into the host conversation for authorship. A bounded list of artifacts already built into the public demo is a deployment manifest, not a hidden composer or an official work library.

The URL Player is a listening space rather than an artifact dossier nested inside a playlist page. One uninterrupted music visualization remains the primary visual object, including for simple airs; it is not replaced by a cover and progress bar. The queue opens on demand without narrowing the score. Previous/play/next, seek and playback mode belong to one transport. Passage/motif context and technical facts are progressively disclosed under **Inside this air**, while direct motif selection and section navigation remain on the listening surface. A theme sets the whole space’s material and typography; the four canonical musical composers keep their distinct visual grammars. The compact MCP delivery remains host-sized.
